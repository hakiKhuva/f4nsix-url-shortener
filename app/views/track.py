...
def index():
    ...
    if tracking_id is not None:
        ...
        if len(tracking_id) <= BaseConfig.TRACKING_ID_MAX_LIMIT:            
            ...
            flash("Tracking id length must be less than or equal to {}!".format(BaseConfig.TRACKING_ID_MAX_LIMIT))


    return modified_render_template(
        "track/index.html",
        page_title="Track ID",
        page_description="Tracking id is the unique string that can be used to track the shortened URL. Tracking id can be used to track the clicks, countries data, devices and other details of the shortened URL with the tracking-id.",
        tracking_data=tracking_data,
        tracking_id=tracking_id,
        grid_data_to_display=["browsers", "devices", "operating-systems", "top-referrers"],
        MAX_LENGTH_FOR_TRACKING_ID=BaseConfig.TRACKING_ID_MAX_LIMIT
    )
